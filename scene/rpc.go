...
import (
	...
	gocraft "github.com/informeai/gocraft-server/client"
...
func GetClient() *gocraft.Client {
	return client
}

func ClientFetchChunk(id internal.Vec3, f func(bid internal.Vec3, w int)) {
	if client == nil {
		return
	}
	...
	for _, b := range rep.Blocks {
		f(internal.Vec3{b[0], b[1], b[2]}, b[3])
	}
...
func ClientUpdateBlock(id internal.Vec3, w int) {
	if client == nil {
		return
	}
	cid := id.Chunkid()
...
func (s *BlockService) UpdateBlock(req *proto.UpdateBlockRequest, rep *proto.UpdateBlockResponse) error {
	...
	bid := internal.Vec3{req.X, req.Y, req.Z}
