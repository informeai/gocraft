...
func (s *Store) UpdateBlock(id internal.Vec3, w int) error {
...
func (s *Store) RangeBlocks(id internal.Vec3, f func(bid internal.Vec3, w int)) error {
	return s.db.View(func(tx *bolt.Tx) error {
		bkt := tx.Bucket(blockBucket)
		startkey := encodeBlockDbKey(id, internal.Vec3{0, 0, 0})
		iter := bkt.Cursor()
...
func (s *Store) UpdateChunkVersion(id internal.Vec3, version string) error {
	return s.db.Update(func(tx *bolt.Tx) error {
...
func (s *Store) GetChunkVersion(id internal.Vec3) string {
	var version string
	s.db.View(func(tx *bolt.Tx) error {
		bkt := tx.Bucket(chunkBucket)
		key := encodeVec3(id)
...
func encodeVec3(v internal.Vec3) []byte {
	buf := new(bytes.Buffer)
	binary.Write(buf, binary.LittleEndian, [...]int32{int32(v.X), int32(v.Y), int32(v.Z)})
	return buf.Bytes()
}

func encodeBlockDbKey(cid, bid internal.Vec3) []byte {
	buf := new(bytes.Buffer)
...
func decodeBlockDbKey(b []byte) (internal.Vec3, internal.Vec3) {
	if len(b) != 4*5 {
	...
	cid := internal.Vec3{int(arr[0]), 0, int(arr[1])}
	bid := internal.Vec3{int(arr[2]), int(arr[3]), int(arr[4])}
	if bid.Chunkid() != cid {
