...
import (
	...
	"github.com/informeai/gocraft/shader"
)
...
func NewBlockRender() (*BlockRender, error) {
	...
	mainthread.Call(func() {
		...
		}, shader.BlockVertexSource, shader.BlockFragmentSource)
		fmt.Printf("R SHADER: %+v\n", r.shader)
		fmt.Printf("ERROR: %+v\n", err)
		if err != nil {
			return
		}
...
func (r *BlockRender) makeChunkMesh(c *internal.Chunk, onmainthread bool) *Mesh {
	facedata := r.facePool.Get().([]float32)
	defer r.facePool.Put(facedata[:0])

	c.RangeBlocks(func(id internal.Vec3, w int) {
		if w == 0 {
		...
		if IsPlant(game.world.Block(id)) {
			facedata = internal.MakePlantData(facedata, show, id, internal.Tex.Texture(w))
		} else {
			facedata = internal.MakeCubeData(facedata, show, id, internal.Tex.Texture(w))
		}
	})
...
func (r *BlockRender) UpdateItem(w int) {
	...
	texture := internal.Tex.Texture(w)
	show := [...]bool{true, true, true, true, true, true}
	pos := internal.Vec3{0, 0, 0}
	if IsPlant(w) {
		vertices = internal.MakePlantData(vertices, show, pos, texture)
	} else {
		vertices = internal.MakeCubeData(vertices, show, pos, texture)
	}
...
func isChunkVisiable(planes []mgl32.Vec4, id internal.Vec3) bool {
	p := mgl32.Vec3{float32(id.X * internal.ChunkWidth), 0, float32(id.Z * internal.ChunkWidth)}
	const m = internal.ChunkWidth
...
func (r *BlockRender) get3dmat() mgl32.Mat4 {
	n := float32(*renderRadius * internal.ChunkWidth)
	width, height := game.win.GetSize()
	mat := mgl32.Perspective(internal.Radian(45), float32(width)/float32(height), 0.01, n)
...
func (r *BlockRender) sortChunks(chunks []internal.Vec3) []internal.Vec3 {
	cid := internal.NearBlock(game.Camera.Pos()).Chunkid()
	x, z := cid.X, cid.Z
	mat := r.get3dmat()
...
func (r *BlockRender) updateMeshCache() {
	block := internal.NearBlock(game.Camera.Pos())
	chunk := block.Chunkid()
	x, z := chunk.X, chunk.Z
	n := *renderRadius
	needed := make(map[internal.Vec3]bool)

	for dx := -n; dx < n; dx++ {
		for dz := -n; dz < n; dz++ {
			id := internal.Vec3{x + dx, 0, z + dz}
			if dx*dx+dz*dz > n*n {
				continue
			}
			needed[id] = true
	...
	var added, removed []internal.Vec3
...
func (r *BlockRender) forceChunks(ids []internal.Vec3) {
	var removedMesh []*Mesh
...
func (r *BlockRender) forcePlayerChunks() {
	bid := internal.NearBlock(game.Camera.Pos())
	cid := bid.Chunkid()
	var ids []internal.Vec3
	for dx := -1; dx <= 1; dx++ {
		for dz := -1; dz <= 1; dz++ {
			id := internal.Vec3{cid.X + dx, 0, cid.Z + dz}
			ids = append(ids, id)
		}
	}
	r.forceChunks(ids)
...
func (r *BlockRender) DirtyChunk(id internal.Vec3) {
	mesh, ok := r.meshcache.Load(id)
	if !ok {
		return
	}
...
func (r *BlockRender) drawChunks() {
	...
	r.shader.SetUniformAttr(2, float32(*renderRadius)*internal.ChunkWidth)

	planes := frustumPlanes(&mat)
	r.stat = Stat{}
	r.meshcache.Range(func(k, v interface{}) bool {
		id, mesh := k.(internal.Vec3), v.(*Mesh)
		r.stat.CacheChunks++
...
func (r *BlockRender) drawItem() {
	...
	model = model.Mul4(mgl32.HomogRotate3DX(internal.Radian(10)))
	model = model.Mul4(mgl32.HomogRotate3DY(internal.Radian(45)))
...
func NewLineRender() (*LineRender, error) {
	...
	mainthread.Call(func() {
		r.shader, err = glhf.NewShader(glhf.AttrFormat{
			...
			glhf.Attr{Name: "matrix", Type: glhf.Mat4},
		}, shader.LineVertexSource, shader.LineFragmentSource)
...
func (r *LineRender) drawWireFrame(mat mgl32.Mat4) {
	...
	vertices = internal.MakeWireFrameData(vertices, show)
	if len(vertices) == 0 {
		return
	}
	r.lastBlock = *block
...
func (r *LineRender) Draw() {
	width, height := game.win.GetSize()
	projection := mgl32.Perspective(internal.Radian(45), float32(width)/float32(height), 0.01, internal.ChunkWidth*float32(*renderRadius))
